 ...
 */
 ...
 public class PokemonInsercao {
     ...
     public Pokemon(int id, int generation, String name, String description, ArrayList<String>  types, ArrayList<String>  abilities, double weight, double height, int captureRate, boolean isLegendary, LocalDate captureDate) {
         this.id = id;
     ...
     public void printPokemon(){
         ...
         String formattedDate = this.captureDate.format(outputDateFormat);
     ...
     public static boolean isFim(String entrada){
         ...
         public static Pokemon criarPokemon(String linha) {
             ...
             DateTimeFormatter ddf = DateTimeFormatter.ofPattern("dd/MM/yyyy");
             ...
             try {
                pokemon.setCaptureDate(LocalDate.parse(campos[11], ddf));
            } catch (Exception e) {
                 e.printStackTrace();
    ...
    public static int[] ordenacaoInsercao(Pokemon pokemon[], int n){
        ...
        int dados[] = new int[2];
            ...
            for(int i = 1; i < n; i++){
                ...
                dados[0] = dados[0]+ 6;
                while( (j >= 0) 
                        && ((pokemon[j].captureDate.getYear() > tmp.captureDate.getYear())
                        || (pokemon[j].captureDate.getYear() == tmp.captureDate.getYear() && pokemon[j].captureDate.getMonthValue() > tmp.captureDate.getMonthValue())
                        || (pokemon[j].captureDate.getYear() == tmp.captureDate.getYear() && pokemon[j].captureDate.getMonthValue() == tmp.captureDate.getMonthValue() && pokemon[j].captureDate.getDayOfMonth() > tmp.captureDate.getDayOfMonth()))){
     ...
     public static void main(String[] args) {
        ...
        int dados[] = ordenacaoInsercao(pokemonsBuscados, i);
        ...
        for(int j = 0; j < i; j++){
         ...
         GravarArquivoDeExecucao("857859_insercao.txt", dados[0], dados[1], FimTime); //Arquivo de analise de Execução
